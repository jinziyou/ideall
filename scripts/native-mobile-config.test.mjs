...
test("iOS 宿主在 UIKit 激活后启动 GPUI 并串行化状态通知", () => {
  ...
  assert.match(main, /input\.alpha = 1\.0/)
})
...
test("移动 CI 驱动真实输入、旋转和前后台恢复", () => {
  ...
  assert.match(iosTest, /app\.keyboards\.firstMatch\.waitForExistence/)
  assert.match(iosTest, /app\.typeText\("ideall iOS smoke body/)
  assert.match(iosTest, /app\.typeText\("ideall iOS smoke title/)
  ...
  assert.match(androidSmoke, /screen_height \* 11 \/ 100/)
