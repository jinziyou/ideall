...
test("iOS 宿主在 UIKit 激活后启动 GPUI 并串行化状态通知", () => {
  ...
  assert.match(main, /\[rootView addSubview:proxy\]/)
...
test("移动 CI 驱动真实输入、旋转和前后台恢复", () => {
  ...
  assert.match(iosTest, /through: "新建笔记"/)
  assert.match(iosTest, /through: "聚焦标题"/)
  assert.match(iosTest, /through: "聚焦正文"/)
  assert.match(iosTest, /let proxy = app\.buttons\[proxyLabel\]/)
