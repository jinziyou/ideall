...
test("移动 CI 驱动真实输入、旋转和前后台恢复", () => {
  ...
  assert.match(iosTest, /app\.textViews\.firstMatch/)
  assert.match(iosTest, /launchEnvironment\["IDEALL_UI_TESTING"\] = "1"/)
  assert.match(iosTest, /nativeInput\.waitForExistence/)
  assert.match(iosTest, /nativeInput\.typeText\("ideall iOS smoke body/)
  assert.match(iosTest, /nativeInput\.typeText\("ideall iOS smoke title/)
  ...
  assert.match(androidSmoke, /tap_y_percent=\$\(\(5 \+ \(\(attempt - 1\) % 3\)\)\)/)
