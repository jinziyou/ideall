...
mod tests {
    ...
    fn declared_oauth_commands(source: &str) -> Vec<&str> {
        let attribute = concat!("#[tauri::", "command]");
        source
            .split(attribute)
            .skip(1)
            .filter_map(|tail| {
                let (_, after_fn) = tail.split_once("fn ")?;
                let name_len = after_fn
                    .bytes()
                    .take_while(|byte| byte.is_ascii_alphanumeric() || *byte == b'_')
                    .count();
                (name_len > 0).then_some(&after_fn[..name_len])
            })
            .collect()
    }

    #[test]
    fn oauth_commands_are_registered_and_authorized_for_main_desktop_window() {
        let commands = declared_oauth_commands(include_str!("oauth_callback.rs"));
        assert!(!commands.is_empty(), "OAuth module must expose commands");
        ...
        for command in commands {
            assert!(
                invoke_handler.contains(&format!("oauth_callback::{command},")),
                "{command} is not registered in the desktop invoke handler"
            );
            assert!(
                build_manifest.contains(&format!("\"{command}\"")),
                "{command} is missing from build.rs AppManifest ACL generation"
            );

            let permission = format!("allow-{}", command.replace('_', "-"));
            assert!(
                permissions.iter().any(|item| item == &permission),
                "desktop capability does not grant {permission}"
            );

            let generated_path = Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("permissions/autogenerated")
                .join(format!("{command}.toml"));
            let generated = std::fs::read_to_string(&generated_path).unwrap_or_else(|error| {
                panic!(
                    "generated ACL permission is missing at {}: {error}",
                    generated_path.display()
                )
            });
            assert!(
                generated.contains(&format!("commands.allow = [\"{command}\"]")),
                "generated ACL permission does not allow {command}"
            );
        }
    }
