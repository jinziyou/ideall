...
> 决策（2026-06）：轴 A/B/C **均对外可添加**。轴 C 端口后端按端口分信任档——`ServerPort`/`SyncPort` 用户可换（中立主张核心；`SyncPort` 仅见密文），`FilesPort`（全部本地数据明文）默认仅自托管/高级项。详见 [extension-registry-design.md §2.4](extension-registry-design.md)。
...
| **Prompts**（AI 工作流 / 模板） | `skill` | **未实现**（`bridge §5.3` 文档先行，`src` 内零命中） | 补 `server.prompt` + 映射 `agent.run` |
