...
- **联网发现客户端已落地**：桌面设置通过 Settings FileSystem 显式刷新固定官方 Registry；Rust 对签名信封的原始 payload 逐页验签，限制游标链、页数、条目数、有效期与 HTTPS 包地址，并原子缓存原始签名页。离线只回退到每次重新验签的缓存，过期状态会明确显示。目录条目只能打开签名包地址；安装仍走本机 envelope 的完整复验和逐扩展 consent。签名 Release 已发布并独立验签通过；Wonita 生产端点仍返回 HTTP 404，等待部署已合并的代理路由后完成端到端验收。
...
- **生产 Feed 发布链已激活**：ideall 以现有 updater 根在 Actions 中生成确定性分页信封，经 staging Release 原子切换固定通道；首次空目录 Release 已绑定稳定 main，下载摘要与 Minisign 独立验收通过。Wonita 服务端代理代码已合并且不接触私钥，剩余步骤是生产部署和真实端点/桌面刷新验收。
