...
- **CORS / 鉴权**：App 经 `tauri-plugin-http` 绕过 CORS（Phase 3，agent 已用）；调 super/server 时若用浏览器 `fetch`，super 侧 `CORS_ALLOW_ORIGINS` 仍需放行 app/web 来源（属 super 配置），或同样改用插件 fetch。
