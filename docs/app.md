...
```
myos/                         # 仓库根 (wonita monorepo 内挂载于 peer/)
...
- **CORS / 鉴权**：Web 浏览器取数经同源 `/api/backend` 代理（`app/api/backend/[...path]/route.ts`）转发到 super/server，规避跨域与构建期内联生产域名；App 静态导出无该代理，客户端直连 `NEXT_PUBLIC_SERVER_ADDR`。App 经 `tauri-plugin-http` 绕过 CORS（Phase 3，agent 已用）；调 super/server 时若用浏览器 `fetch`，super 侧 `CORS_ALLOW_ORIGINS` 仍需放行 app/web 来源（属 super 配置），或同样改用插件 fetch。
