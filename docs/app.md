...
换个端口启动即可绕开，**无需改动仓库文件**——用 Tauri 的 `--config` 内联覆盖 `devUrl` 与 `beforeDevCommand`：

```bash
pnpm tauri dev --config '{"build":{"devUrl":"http://localhost:5026","beforeDevCommand":"pnpm exec next dev -p 5026"}}'
```

- `beforeDevCommand` 改为 `pnpm exec next dev -p 5026` → 前端开发服起在 5026；
- `devUrl` 同步指到 5026 → Tauri 壳加载它；
- 端口随意换（5026 仅示例，挑一个 `ss -ltn` 里没占用的即可）。
