...
- **激活条件**：自动更新只认**已发布的「非草稿、非预发布」tag 版**。流程：**先 `pnpm bump <x.y.z>` 一键同步版本号并提交**（覆盖 `package.json` / `src-tauri/tauri.conf.json` / `Cargo.toml` / `Cargo.lock` 四处；tauri-action 按 `tauri.conf.json` 的 version 生成 `latest.json`，忘 bump 会让自动更新判定「无更新」；CI 的 gate 已加一致性卡点，不一致直接失败）→ push `app-v*` tag → 出**草稿** Release（已签名）→ 人工 **publish（去草稿）** → 它成为 `releases/latest` → 装机端「检查更新」即拉到。`app-edge` 预发布也带 `.sig`/`latest.json`，但被 `releases/latest` 排除、不用于自动更新（仅供手动下载试用）。
