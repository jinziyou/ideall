# ideall：Web + App（跨平台）方案与路线图

ideall 一套 Next.js 代码库，产出两种形态：

| 形态 | 构建 | 渲染 | 部署 | 数据 |
| --- | --- | --- | --- | --- |
| **Web** | `next build`（`output: standalone`，默认） | SSR + Server Components | **由 ideall 自身的部署流程编排**，经 `SERVER_ADDR` / `NEXT_PUBLIC_SERVER_ADDR` 指向后端数据服务 | 服务端取数（现状） |
| **App** | `BUILD_TARGET=app next build`（`output: export` → `out/`），Tauri 打包 | 静态导出（无 Node 运行时） | 随 ideall 发版，多平台产物 | **客户端直连后端**（CORS + JWT）|
...
- Web 生产部署由 ideall 自身的部署流程编排（`scripts/prod.sh`、`scripts/deploy.sh` 在本仓内构建），经 `SERVER_ADDR` / `NEXT_PUBLIC_SERVER_ADDR` 指向后端数据服务。
...
1. `pnpm tauri signer generate -w ~/.tauri/ideall.key`（生成更新签名密钥对，**私钥保密**）。
