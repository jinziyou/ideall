...
- **一切皆视图**：默认引擎在主窗口标签页显示；用户选择其他引擎时创建独立窗口。标签身份为“文件 + 引擎”。
...
`WsMode = local | connected` 是独立的数据来源镜头，只过滤同一个合成文件系统中当前可见的根目录。切换工作区不改变 mode、当前 `FileRef`、标签或脏草稿；切换 mode 也不卸载 provider。旧的 AI `workspace` 目录只为 FileRef、深链和持久化兼容保留，并标记为导航隐藏；AI 对话从文件工作区的右侧 Agent 面板到达，MCP、Skills 与规则从 AI 设置内的管理入口打开。
...
**一切皆文件（工作区，`src/workspace/*`）**：打开任意目标都会先规范成 `FileRef`，再由 Engine registry 解析渲染器并落入统一文件标签。目录树通过 FileSystem `readDirectory/stat/watch` 展示合成根子树；旧静态 tab、Resource tab、Node tab 与深链在打开或水合时迁移为 File + Engine，运行期不再维护第二套文件身份。工作区只改变 Display 组合和无显式偏好时的 Engine 默认选择，不复制文件或创建另一套导航。
...
| **workspace** | `@/workspace/*` | Display 编排——`workspace-shell` / `workspace-switch` / `workspace-dock` 组合三种工作区；`tab-host` 保持挂载多标签，`registry` 与 `file-engine-renderer` 渲染 File + Engine，`tree/file-system-sidebar-tree` 展示目录，`store` 保存模式、工作区和标签状态，并提供活动栏 / 二级侧栏 / 标签条 / 状态栏等 IDE chrome。 |
| **filesystem** | `@/filesystem/*` | 多实例文件系统 registry、隐藏合成根、旧 Resource 适配器及动态 App 挂载。统一 `stat/readDirectory/read/write/actions/watch`，目录项与文件身份分离。 |
