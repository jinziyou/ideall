...
本机内容、远端资源、系统面板与 App 挂载共享同一导航体系。切换工作区不改变当前 `FileRef`、既有标签或脏草稿；活动项是文件时，会激活同一 `FileRef` 在新场景下的默认 Engine 标签，旧 Engine 标签保留。旧的 AI `workspace` 目录只为 FileRef、深链和持久化兼容保留，并标记为导航隐藏；AI 对话从文件工作区的右侧 Agent 面板到达，MCP、Skills 与规则从“设置 > AI”内的管理入口打开。
