...
ideall 是**开源、本地优先、供应商中立的个人信息终端**：把分散的他人、信息、资源、工具，从你自己的视角聚合到一处。它**仅以 App 形态分发**——同一套 Next.js 代码经 Tauri 2.0 静态导出后打包为跨平台客户端。
...
**设计思想：一切皆文件，一切皆标签页**（在上述三条之上的组织/交互层，已落地，详见 [design/ai-native-redesign.md](design/ai-native-redesign.md)）：
...
| AI 对话线程 `Thread` | 本地 | `protocol/files.ts` | BYO-key AI 助手；消息内联存于线程节点，经 `FilesPort` 读写（不再由 agent 插件自存） |
...
home（即「我的」）是本机数据区；info / community / tool 是三个**发现**模块——它们发现的内容经**订阅 / 关注回流到「我的」**的 `/home/subscriptions` 订阅流。

| 模块 | 路由 | 角色 | 是否需后端 |
| --- | --- | --- | --- |
| **home（「我的」）** | `/home`（笔记 / 书签 / 资源 / 订阅 / 对话；工作区标签页 + places 侧栏） | 「我的」：本机数据区，本地内容工作区，统一 Node 库 + 一切皆标签页 | 否（本地优先） |
...
**一切皆标签页（工作区，`src/workspace/*`）**：打开任意节点即开一个工作区标签页，标签内容由 kind→渲染注册表（`registry`）决定（已落地 note/file/bookmark/feed/thread 五个 kind 查看器）；深链用 `?node=kind:id`（`node-tab`/`parseNodeParams` 编解码，URL 守护幂等）。活动栏「我的」子项是**根命名空间**（places，`NS_ROOT`：笔记/书签/资源/订阅/对话），侧栏是基于泛型 `buildTree<T extends TreeItem>` 渲染的**跨 kind 只读文件树**（`places`/`node-tree`）。`MODULE_OF_KIND` 把全部本地 kind 归 `home`（「我的」）。
...
| 层 | 别名 | 内容 |
| --- | --- | --- |
| **app** | `@/app/*` | Next 路由层——仅「开标签」薄标记：`page.tsx` 全是 re-export `@/workspace/open-workspace-tab`，外加根 `layout`/`error`/`loading`/`not-found` + `globals.css`（`@/app/globals.css`）。 |
| **shell** | `@/shell/*` | 终端外壳——活动栏 / 二级侧栏 / 标签条 / 状态栏 / 命令台 / 主题 / account / mobile-nav / nav-config，加 `boot`（组合根，**唯一**允许 import 各 manifest 处）/ `boot-gate`（启动闸）。 |
| **workspace** | `@/workspace/*` | 一切皆标签页——`tab-host`（keep-alive 多标签）/ `registry`（kind→渲染）/ `store` / `viewers`（按 kind：note/file/bookmark/feed/thread）/ `modules`（模块配置单一真相源）/ `node-ref` / `node-tab` / `node-tree` / `places`（根命名空间）/ `local-search`。 |
...
**端口模式**：每个跨模块契约都是「端口 + register/get」。模块经 protocol 间接协作，外壳与工作区永不直连具体 module/plugin：
...
- **「我的」数据**：反馈组件、agent 插件与 AI `fs.*` 层经 `@protocol/files` 的 `getFilesPort()`（FilesPort，「我的」在 boot 注册实现）读写笔记/订阅/书签/资源/线程，不直接依赖底层存储。FilesPort 已扩出节点级 `fsListNodes/fsGetNode/fsCreateNode/fsUpdateNode/fsMoveNode/fsDeleteNode/fsReadBlob` + 线程方法。
...
- **能力链路**：`createLocalMcpServer(grant, ctx)` 据 `grant.permissions` 注册工具；权限位 `fs:read`/`fs:write`/`fs.notes:read`/`fs.notes:write`/`ui.tabs`。工具：`fs.list`/`fs.read`/`fs.readBlob`/`fs.create`/`fs.write`/`fs.move`/`fs.delete` + `ui.openTab`/`ui.closeTab`，资源 `fs://nodes`。
- **agent→MCP 回环**：agent 插件经 `agentGrant`（`fs:read`/`fs:write`/`fs.notes:write`/`ui.tabs`，**无 `fs.notes:read`**）→ `createLocalMcpServer` → `createLoopbackTransports`（本进程 MessageChannel + 复用的 `MessagePortTransport`）→ `agent-mcp.ts`（`connectAgentMcp`：tools/list→OpenAI 工具、callTool、`summarizeTool` 中文回执）。即 agent 与嵌入第三方走同一套受限工具面，无特权旁路。
...
1. **本地 IndexedDB（统一 Node 库）**：home/tool 经 FilesPort 读写**单一对象仓 `STORE_NODES`**（六类 kind 节点）+ 旁存 `STORE_BLOBS`（`{key,blob}`，文件原始字节，不进同步）。跨多仓的原子写经 `idbPutAcrossStores`（防孤儿 Blob）；写靠 put→delete 排空、`onupgradeneeded` 内零 I/O 的懒迁移（DB_VERSION 已升至 10 以淘汰旧端缓存）。明文不上传。
...
4. **BYO-key agent 经 fs.* MCP 读写 + Tauri 绕 CORS**：agent 经 `agentGrant`→`createLocalMcpServer`→loopback MCP 客户端（`agent-mcp.ts`）以 `fs.*`/`ui.*` 工具读写 Node 库（与嵌入第三方同一受限工具面），再直连用户配置的 OpenAI 兼容端点。key 仅存本地；在 App（Tauri）内经 `tauri-plugin-http`（Rust 侧请求）绕过 webview CORS，可直连任意云厂商端点；纯浏览器调试时用标准 fetch，受厂商 CORS 限制（本地 Ollama / 放行 CORS 的端点可用）。
