...
- **双模式统一导航**：隐藏合成根始终汇总全部来源；Display 按 local/connected 模式过滤其直接子树，模式切换只改变导航镜头，不改变文件身份、挂载或已打开标签。
...
> `home/apps/info/community/tool` 仍是业务实现与旧路由的代码边界，不再直接生成桌面活动栏。Display 从合成文件系统根生成活动栏，并按本地/连接数据来源模式提供两个导航镜头；这些模块通过系统文件、Resource 兼容适配器或 App 文件系统被打开。

### 3.1 三种工作区与双数据来源模式

工作区与视图是同一个概念，由 `WorkspaceKind` 表达，只在桌面顶栏或移动顶栏切换，不进入活动栏、目录树或移动导航抽屉：
...
| **开发** | 文件工作区 + 保持挂载的 Git / Shell 工具区 | 通用文本文件优先使用 Code Engine |

`WsMode = local | connected` 是独立的数据来源镜头，只过滤同一个合成文件系统中当前可见的根目录。切换工作区不改变 mode、当前 `FileRef`、既有标签或脏草稿；活动项是文件时，会激活同一 `FileRef` 在新场景下的默认 Engine 标签，旧 Engine 标签保留。切换 mode 也不卸载 provider。旧的 AI `workspace` 目录只为 FileRef、深链和持久化兼容保留，并标记为导航隐藏；AI 对话从文件工作区的右侧 Agent 面板到达，MCP、Skills 与规则从 AI 设置内的管理入口打开。
...
>
> **浏览器**是连接模式下的文件系统入口，打开 `browser:page:default` Resource。**AI**不是左侧导航工作区；它是三种工作区共享的 Agent 能力，由右侧对话栏、设置入口和 ai-* 兼容文件承载。

**一切皆文件（工作区，`src/workspace/*`）**：打开任意目标都会先规范成 `FileRef`，再由 Engine registry 解析渲染器并落入统一文件标签。目录树通过 FileSystem `readDirectory/stat/watch` 展示合成根子树；旧静态 tab、Resource tab、Node tab 与深链在打开或水合时迁移为 File + Engine，运行期不再维护第二套文件身份。异步文件打开采用按通道隔离的 last-request-wins，并在 `stat` 完成后按最新工作区解析默认 Engine，避免慢 provider 用旧场景抢回焦点。标签迁移、生命周期/LRU 策略和导航请求协调已从 store 副作用 facade 抽成纯模块。可序列化 dirty Engine 只有在有界、身份绑定的 session 快照成功后才允许被 LRU 休眠；失败时保持运行。工作区只改变 Display 组合和无显式偏好时的 Engine 默认选择，不复制文件或创建另一套导航。
...
| **workspace** | `@/workspace/*` | Display 编排——`workspace-shell` / `workspace-switch` / `workspace-dock` 组合三种工作区；`tab-host` 保持挂载多标签并只休眠已有安全快照的 dirty Engine，`registry` 与 `file-engine-renderer` 渲染 File + Engine，`tree/file-system-sidebar-tree` 展示目录；`store` 是状态/副作用 facade，迁移兼容、标签生命周期和导航竞态策略分别收敛到纯模块。 |
| **filesystem** | `@/filesystem/*` | 多实例文件系统 registry、隐藏合成根、旧 Resource 适配器及动态 App 挂载。统一 `stat/readDirectory/read/readMany/write/actions/watch`，目录项与文件身份分离；批量读取按 provider 分组并有限并发回退，provider generation 变化会拒绝迟到结果。registry 共享相同 watch、拥有注销/替换清理并用 generation 丢弃旧回调；音频/数据库增量事件按 target 与 old/new parent 路由且保留 parent-scoped `entryId` link 身份，目录 Display 在安全条件下做单项更新并保守回退全量读取。 |
...
8. **统一 Node 库的投影封边界**（一切皆文件）：所有本地内容存单一 `STORE_NODES`，节点↔域类型投影只在 provider/storage 侧发生；普通消费方经 FileSystem 或其 FilesPort 兼容外观访问，不能直接 import store。含墓碑快照与原子 bulk 的例外只向 sync 暴露为 `StorageSyncPort`。`onupgradeneeded` 仅建仓、**绝不做 I/O**。
