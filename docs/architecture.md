...
> `home/apps/info/community/tool` 仍是业务实现与旧路由的代码边界，不再直接生成桌面活动栏。Display 从合成文件系统根生成活动栏，并按本地/连接数据来源模式提供两个导航镜头；这些模块通过系统文件、`filesystem/resource-sources` 中的 Resource 兼容适配器或 App 文件系统被打开。
