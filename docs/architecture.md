...
- **统一导航**：隐藏合成根，活动栏展示根的直接子树，不再用 local/connected 模式过滤两组模块。
...
> `home/apps/info/community/tool` 仍是业务实现与旧路由的代码边界，不再是桌面活动栏的一级产品分区。Display 统一从合成文件系统根生成活动栏；这些模块通过系统文件、Resource 兼容适配器或 App 文件系统被打开。
...
| **engines** | `@/engines/*` | 纯引擎描述、匹配/默认解析、单文件与 media type 偏好；具体 React renderer 仍由 workspace Display 注册。 |
