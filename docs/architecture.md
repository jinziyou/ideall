...
| **workspace** | `@/workspace/*` | 一切皆标签页——`tab-host`（keep-alive 多标签）/ `registry`（kind→渲染）/ `store` / `viewers`（按 kind：note/file/bookmark/feed/thread）/ `modules`（模块配置单一真相源）/ `node-ref` / `node-tab` / `node-tree` / `places`（根命名空间）/ `local-search`，以及活动栏 / 二级侧栏 / 标签条 / 状态栏 4 件 IDE chrome。 |
