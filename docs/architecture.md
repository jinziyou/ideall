...
home（即「我的」）是本机数据区；info / community / tool 是三个**发现**模块——它们发现的内容经**关注汇入「我的」**的 `/home/subscriptions` 关注流。
...
**一切皆标签页（工作区，`src/workspace/*`）**：打开任意节点即开一个工作区标签页，标签内容由 kind→渲染注册表（`registry`）决定（已落地 note/file/bookmark/feed/thread 五个 kind 查看器）；深链用 `?node=kind:id`（`node-tab`/`parseNodeParams` 编解码，URL 守护幂等）。活动栏「我的」子项是**根命名空间**（places，`NS_ROOT`：笔记/书签/资源/关注/对话），侧栏是基于泛型 `buildTree<T extends TreeItem>` 渲染的**跨 kind 只读文件树**（`places`/`node-tree`）。`MODULE_OF_KIND` 把全部本地 kind 归 `home`（「我的」）。
