...
**一切皆标签页（工作区，`src/workspace/*`）**：打开任意节点即开一个工作区标签页，标签内容由 kind→渲染注册表（`registry`）决定（节点查看器已落地 note/file/bookmark/feed/thread 五个 kind）；`registry` 也分派**非 node 的标签类型**——典型是 `browser-view`（内嵌浏览器，原生子 webview，`src/workspace/browser-view.tsx` + `src-tauri/src/browser_linux.rs` + `src-tauri/capabilities/browser.json`）；它**刻意留在 workspace 层**（有意例外：其余标签的功能 UI 都在 `modules/`，而 browser 与 workspace 的 store / tauri 桥强耦合，故不外移）。深链用 `?node=kind:id`（`node-tab`/`parseNodeParams` 编解码，URL 守护幂等）。活动栏「我的」子项是**根命名空间**（places：笔记/书签/资源/关注/对话），侧栏是基于泛型 `buildTree<T extends TreeItem>` 渲染的**跨 kind 文件树**（书签/收藏夹支持拖拽重排），实现现收于 `src/workspace/tree/`（`sidebar-tree.tsx` / `sidebar-tree-data.ts` / `sidebar-tree-node-branch.tsx` / `sidebar-tree-bus.ts` / `draggable-node-forest.tsx` / `home-sections.ts`）。`MODULE_OF_KIND` 把全部本地 kind 归 `home`（「我的」）。
