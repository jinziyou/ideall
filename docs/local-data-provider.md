...
> **状态:P1 已落地;P2/P3 仍为设计草图(按需,parked)。**
> **P1（最稳/最高杠杆，已实现）**：把原先糅在 `host.tsx` 里的「能力 / transport / 会话授权」三件事拆开 ——
> 抽出 transport 无关的能力层工厂 `createHubMcpServer(grant)`（`src/components/embed/hub-mcp-server.ts`）+
> 显式 L3 **Grant 模型**（`src/components/embed/grant.ts`：`Grant` / `firstPartyGrant` / `isGrantActive` / `activePermissions`，含单测 `grant.test.ts`）。
> `host.tsx` 由此退化为「iframe transport 绑定」之一：构造一方 **T0 Grant**（自动/不过期/不可撤，保留现状语义）→ 调工厂拿 server → 接 `MessagePortTransport`。
> 行为对 iframe 路径不变；安全不变量（§1）不因抽取而变。**P2（新 transport + consent UI）/ P3（任意源）仍未排期**，下文为其设计。
>
> 缘起:讨论「能否让数据流反向 —— 让外部消费方(如 wonita portal)请求 ideall 中存的本地用户数据」,以及「浏览器插件是不是这个思路」。结论是:**反向取数在 iframe 嵌入语境里已经实现**(`src/components/embed/` 的 MCP 宿主桥),本文设计如何把它**泛化**成传输无关的「本地数据提供方」并配套 consent/权限模型。

## 背景:反向取数已存在于嵌入桥

`src/components/embed/`(`host.tsx` + `tools.ts` + `manifest.ts` + `protocol.ts`)已实现一个方向:ideall 内嵌 wonita portal(`<iframe src="${NEXT_PUBLIC_EMBED_BASE}/info">`),**ideall 跑 `McpServer`(宿主)、portal 是 MCP client(访客)**,ideall 按 manifest 的 `permissions` 只注册被授权的工具/资源。于是内嵌页能向 ideall「反问」本地数据(订阅/书签/身份),据此渲染「已订阅」「保存到中枢」「发布」。
...
[L3 会话/授权]  认证对端 → 解析 grant → consent 闸 → 限流/限范围   ★新增
        ▼
[L1 能力]   McpServer(只挂 granted) → getHubData / getServerPort / auth-store
        ▼
   本地数据(IndexedDB)  +  ServerPort(token 留宿主侧)
```

- **L1 能力层**(**已抽出**,transport 无关):`host.tsx` 里 `new McpServer + registerGrantedTools/Resources` 已抽成工厂 `createHubMcpServer(grant)`(`hub-mcp-server.ts`),据 Grant 起 server;`host.tsx` 仅余 iframe transport 绑定。
- **L2 传输层**(可插拔):`MessagePortTransport`(现有)+ 新增 `NativeMessagingTransport`、`LoopbackTransport`,都实现 MCP `Transport` 接口。
- **L3 会话/授权层**(新):对端是谁、授了哪些权、是否 consent 过、是否仍有效/可吊销。现状是隐式的(静态 manifest + 一方信任),需显式化才能扩到第三方。
...
把现有工具(`src/components/embed/protocol.ts` 的 `TOOL`)按敏感度归档,决定哪层 grant 能用:
...
反向流仍合「本地优先」当且仅当:就地读不上传 + token 不可被带走 + 每个非一方消费方显式 consent 且可撤 + **中立**(提供方服务任何配对成功的消费方,而非只服务 wonita —— 否则就是反向供应商锁定,自打 ServerPort 中立的脸)。
...
| **重构(已做)** | 从 `host.tsx` 抽出 `createHubMcpServer(grant)`;`host.tsx` 退化为「iframe transport 绑定」之一 |
| **新建** | ~~L3 Grant 模型(类型 + 一方构造 + 过期判定)~~(P1 已做,`grant.ts`);**剩余**:L3 Grant/Consent **持久化存储**(IndexedDB:已连接消费方/权限/过期)+「已连接的应用」设置 UI + 配对流;`NativeMessagingTransport` + Tauri native-host 注册;`LoopbackTransport`(origin 白名单 + capability token + 防 rebinding) |

**分期建议:**
1. **P1(最稳、最高杠杆)——✅ 已落地**:已抽 `createHubMcpServer` + 引入显式 L3 Grant **类型/构造/过期判定**(`grant.ts` + `hub-mcp-server.ts` + `grant.test.ts`),不加新 transport,把 iframe 路径隐式的一方信任显式化,为未来扩展/回环铺路,完全不碰 Web 源暴露风险。**注:P1 只做了内存态 Grant 模型,Grant 的 IndexedDB 持久化 + consent UI 属 P2。**
2. **P2**:Grant 持久化 + 「已连接的应用」consent UI(T1);`LoopbackTransport` 或 `NativeMessagingTransport` 二选一 + 配对流。
3. **P3(谨慎,按需)**:T2 任意源 / 扩展直通 `web.wonita.link`。

## 8. 待拍板的决策点

1. **要不要 T2(任意 Web 源)?** 倾向先不做,封顶 T1。T2 收益(独立标签页直读)抵不过它撞 H-1 凭证面 + 「数据自持」红线的风险,除非有硬需求。
2. **消费方信任根**:T1 的「已验」靠什么 —— manifest 签名?ideall 内置登记表?(影响中立性:登记表别只收录 wonita。)
...
- 嵌入桥实现:`src/components/embed/`(`host.tsx` / `tools.ts` / `manifest.ts` / `protocol.ts`)。
