...
> 显式 L3 **Grant 模型**（`src/plugins/embed/grant.ts`：`Grant` / `firstPartyGrant` / `isGrantActive` / `activePermissions`，含单测 `grant.test.ts`）。
...
**分期建议:**
1. **P1(最稳、最高杠杆)——✅ 已落地**:已抽 `createLocalMcpServer` + 引入显式 L3 Grant **类型/构造/过期判定**(`grant.ts` + `local-mcp-server.ts` + `grant.test.ts`),不加新 transport,把 iframe 路径隐式的一方信任显式化,为未来扩展/回环铺路,完全不碰 Web 源暴露风险。**注:P1 只做了内存态 Grant 模型,Grant 的 IndexedDB 持久化 + consent UI 属 P2。**
