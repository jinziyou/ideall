...
- 脚本公共能力集中在 `scripts/script-utils.mjs`; 新增验证/冒烟脚本时优先复用其中的命令执行、端口探测、HTTP 就绪等待与子进程清理逻辑
