...
| 目录 | 别名 | 内容 |
|---|---|---|
| **app** | `@/app/*` | Next 路由层 —— 仅「开标签」薄标记 (page.tsx 全是 re-export `@/workspace/open-workspace-tab`) + 根 layout/error/loading/not-found + globals.css |
| **shell** | `@/shell/*` | 终端外壳 —— 命令台 / header / bottom-tab-bar / 主题 (theme/theme-applier) / account / mobile-nav / nav-config + `boot` (组合根, 唯一允许 import 各 manifest 处) / `boot-gate` (启动闸) |
| **workspace** | `@/workspace/*` | 一切皆标签 —— `tab-host` (keep-alive 多标签) / `registry` (kind→渲染) / store / `viewers` (按 kind: note/file/bookmark/feed/thread) / `modules` (模块配置单一真相源) / node-ref / node-tab / node-tree / places (根命名空间) / local-search, 以及活动栏 / 二级侧栏 / 标签条 / 状态栏 4 件 IDE chrome |
