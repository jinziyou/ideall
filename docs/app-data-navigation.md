...
v17 在既有 `kind`、`[kind,sortKey]` 等索引上增加 `[kind,sortKey,title,id]` 覆盖索引。Node 目录可使用稳定 cursor 按 kind 增量读取摘要，不需要先 `getAll()` 整库再在内存分页。
...
2. 检查 IndexedDB 是否存在 `wonita-home`、版本是否为 17。
...
3. **大目录性能**：IndexedDB 升级至 v17，Node 摘要查询和左侧树使用稳定 cursor 增量分页。
