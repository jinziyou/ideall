...
**设计思想**: 一切皆文件、一切皆标签页 —— 各类 Storage 经 FileSystem 挂载为统一命名空间里的
`FileRef`,打开任意文件即由 Engine 渲染为工作区标签页。`ideall.core` 的笔记/书签/资源/关注/对话
收敛为 Node 库;音频、数据库、Agent 配置和第三方 App 等来源保留各自存储语义。
**设计风格**: 现代 · 面板 · 留白的标签工作区。

**战略方向 D（已定）**: ideall **能独立成立**, 命运不与 wonita 深度绑定。
- **本地核心不依赖后端**: 「我的」(home) / tool 本地能力 + BYO-key agent 可离线使用。跨端同步无账号且只传
  密文,但同步动作仍需要可用的 Sync 服务;当前同步范围仅为关注 (`subs`) 与笔记 (`notes`)。
...
| **shell** | `@/shell/*` | 终端外壳 —— 命令台 / header / bottom-tab-bar / 主题 / account / mobile-nav，以及唯一组合根 `boot`、启动闸 `boot-gate` 和可信扩展生命周期 `runtime-extensions` |
