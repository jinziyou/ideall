...
所有 Git 依赖必须使用完整 revision，`Cargo.lock` 必须提交，标准入口与 CI 必须使用
`--locked`。gpui-mobile 快照的来源、本地补丁和升级步骤记录在
[`native/vendor/gpui-mobile/README.md`](../../native/vendor/gpui-mobile/README.md)；其对
wgpu `v29` branch 的间接引用由 lockfile 固定到
`357a0c56e0070480ad9daea5d2eaa83150b79e88`。正式发布前应把该 branch 引用改为精确
revision。依赖升级必须作为单独变更，同时通过五平台构建、输入、渲染、生命周期和数据回归。
