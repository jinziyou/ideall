...
> 状态：原生 Preview 实施中。共享核心与 Linux 桌面纵切已通过本地门禁；Windows、macOS、Android 与 iOS 已配置 CI 构建和模拟器启动 smoke，但本轮 workflow 尚未实际运行，仍需真机/真实宿主验收。当前稳定实现仍以 [architecture.md](../architecture.md) 为准；只有通过本文对应阶段门禁的原生能力才能替代现有实现。
...
本迁移借鉴 OxideTerm 的“UI 与领域服务同进程”方向，但不复制其 GPL-3.0 代码或 vendored GPUI fork。ideall 继续使用 Apache-2.0 许可，并对每个引入的 Zed、GPUI、gpui-component 与 gpui-mobile 文件或 crate 独立核对许可证。
...
| `gpui-mobile` | Git `1d3ec2a1d14a63b74d1f4269340441d4eeada27a` |
| mobile `gpui` / `gpui_wgpu` | Zed `5688167d224b5eca54875d49afb8bfd73a07915a` |

所有 Git 依赖必须使用完整 revision，`Cargo.lock` 必须提交，标准入口与 CI 必须使用 `--locked`。gpui-mobile 当前对 wgpu `v29` branch 的间接引用由 lockfile 固定到 `357a0c56e0070480ad9daea5d2eaa83150b79e88`；正式发布前应在 ideall 控制的 fork 中把 manifest 改为精确 revision。依赖升级必须作为单独变更，同时通过五平台构建、输入、渲染、生命周期和数据回归。
...
gpui-mobile 当前仍缺完整无障碍、原生 View 嵌入和部分文本输入能力。ideall 需要维护最小 fork 或向上游贡献，不能用空 feature 或 demo 行为冒充完成的平台能力。
...
| Android | gpui-mobile 真实 UI 在 host 编译；CI 已配置双 ABI APK/AAB 校验，并在 x86_64 模拟器安装、启动、检查 resumed activity 与 crash buffer；SAF 选择器把 content URI 有界复制到私有缓存；Keystore 与 Android WebView 已接线；受保护手动 workflow 支持 keystore 签名 AAB | CI/签名 workflow 实际结果、真机中文 IME/选区、旋转、后台恢复与 TalkBack |
| iOS | CI 已配置 arm64 simulator release `.app` 构建、安装和启动；Keychain、WKWebView、主线程 DocumentPicker 与 security-scoped 有界复制已接线；受保护手动 workflow 支持发行证书/描述文件和签名 IPA 导出 | CI/签名 workflow 实际结果、真机中文 IME/选区、旋转、后台恢复与 VoiceOver |
| 本地纵切 | 五分区、树、桌面标签、移动恢复、Note/Bookmark/File/Feed、全文搜索、回收站、导入、系统打开可用；Plate 标题/段落/引用/列表/任务/代码/分隔线可通过可逆 Markdown 投影编辑，未改 marks 复用原 JSON，未知块以指纹占位无损保护；移动端提供当前块格式工具栏、可筛选的 slash 块命令菜单和显式上移/下移块重排，并拒绝改写受保护块或已有代码围栏；移动输入模型已测试 CJK、换行过滤、光标中部插入、Home/End/前后删除、完整 emoji grapheme 删除，以及不重复软键盘事件的外接键盘 Backspace/Delete/Enter 兜底 | 移动端以显式重排替代 Web Plate 的精确拖拽手势；gpui-mobile 宿主的原生组合输入与选择句柄仍须真机验证/补强 |
