...
wgpu `v29` branch 的直接与传递引用保持单一 Cargo source identity，并由 lockfile 精确固定到
`357a0c56e0070480ad9daea5d2eaa83150b79e88`；vendor verifier 会拒绝 commit 漂移，避免改为
`rev` 后与 Zed `gpui_wgpu` 的 branch source 并存而编译两套 wgpu。CI 还会从固定
