...
const config = [
  ...
  // 也不得依赖 super/server 的 wire DTO —— 领域类型在 @protocol/server-port 自有定义。
  {
        ...
        "error",
        {
          patterns: [
            ...
            },
            {
              ...
              message:
                "protocol 不得依赖 super/server wire DTO (openapi 生成类型); 领域类型在 @protocol/server-port 自有定义, wire→domain 映射收敛在 components/lib/server 适配器内",
            },
          ],
        },
      ],
  ...
  // wire DTO 边界: super/server 的 openapi 生成类型 (@/components/lib/api/server) 仅允许
