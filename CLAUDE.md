...
ideall 是**开源、本地优先、供应商中立的个人信息工作台**（独立项目）: 从个人视角出发,
把分散的他人、信息、资源、工具聚合到一处。

**战略方向 D（已定）**: ideall **能独立成立**, 命运不与 wonita 深度绑定。
- **零后端即是完整产品**: home / tool 本地能力 + 无账号端到端同步 + BYO-key agent, 不连后端也可用。
- **wonita 是默认且最好的后端选项, 而非命根**: 取数经 `@protocol/server-port` 的 `ServerPort` 契约,
  **后端可换 / 可自建**, wonita 只是默认与参考实现。中立性是核心对外主张, 必须兑现。
...
ideall 仅以 App 形态分发 (见 [docs/app.md](docs/app.md)):
