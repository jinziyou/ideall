...
ideall 是后端数据服务的**外部消费方 / 客户端**：经 `ServerPort` 契约消费 wonita 服务的数据服务 API（wonita 服务是 `ServerPort` 的参考实现；第三方 / 嵌入式 / 局域网节点亦可实现 `ServerPort`）。信息采集 / NLP / 知识图谱 / 鉴权由该后端数据服务提供，ideall 经 `SERVER_ADDR` 连接，不在本仓库范围内；ideall 不被单一后端绑死。

**分支模型（dev / main）**：`main` 稳定/发布（Web 生产实例与 App 版本 tag 基于它），`dev` 集成/日常开发；改动先进 `dev`，CI 在 `main` / `dev` / PR 均运行，稳定后合并 `main` 发布。

## Positioning

ideall 是**本地优先的个人信息总控终端**（独立项目）: 从个人视角出发,
...
- **Web** (默认): `output: standalone` SSR；Web 生产实例由 ideall 自身的部署流程编排, 经 `SERVER_ADDR` / `NEXT_PUBLIC_SERVER_ADDR` 指向后端数据服务。
