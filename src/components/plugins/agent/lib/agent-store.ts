...
// 本模块改经 @protocol/hub-data 的 HubDataPort 消费 (依赖反转: 插件不直接碰 IndexedDB / core 存储)。
...
export async function saveThread(thread: AgentThread): Promise<void> {
  ...
  await getHubData().saveThread(thread)
}

export async function deleteThread(id: string): Promise<void> {
  await getHubData().deleteThread(id)
}

export async function renameThread(id: string, title: string): Promise<void> {
  await getHubData().renameThread(id, title)
}
