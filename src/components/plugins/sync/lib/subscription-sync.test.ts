...
// 此前仅其纯函数零件被隔离单测、编排本身无覆盖 (M-15)。本测以内存 HubDataPort + stub 的
...
import { deriveKeys, encryptJson, decryptJson } from "@/components/lib/sync-crypto"
...
/** 注册一个内存 HubDataPort, 返回其 store (syncNow 仅用 listAllSubscriptions / bulkPutSubscriptions)。 */
function makeHub(initial: Subscription[]) {
  ...
  registerHubData(port as unknown as HubDataPort)
  return { store }
}
