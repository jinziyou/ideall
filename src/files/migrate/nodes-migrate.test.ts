...
test("关注播种: 确定性 id feed:type:key (绝不 genId)", () => {
...
test("关注播种: content 投影 (type/key/favicon + entity 专属字段)", () => {
...
test("关注播种: 含墓碑全量带过来 (deletedAt 保留, 防已删关注复活)", () => {
  const plan = planFeedsSeed([rawSub({ deletedAt: 999 })], new Set(), NOW)!
  assert.equal(plan.puts[0].deletedAt, 999)
})

test("关注投影 round-trip 无损: feedNodeToSub(subToFeedNode(sub)) === sub", () => {
...
test("关注播种: 同级 sortKey 严格递增唯一; 幂等 (已播种不重写, drain 全量)", () => {
  const subs = [
