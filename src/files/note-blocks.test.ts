...
const m = (v: number, by: string, sk: string, del?: number) => ({
  v,
  by,
  sk,
  ...(del ? { del } : {}),
...
test("applyBlockPatch: 只动点名块, 并发追加的 B4 原样保留 (§7.3 反例精确解)", () => {
  ...
  assert.equal(
    (out.content.find((b) => b.id === "B1")!.children as { text: string }[])[0].text,
    "一改",
  )
...
test("applyBlockPatch: v 守卫 —— 陈旧 upsert (v<=现有) 跳过, 不复活 live-merge 的高版本", () => {
  ...
  const patch = {
    upsert: [{ id: "B1", block: blk("B1", "陈旧 base"), v: 2, by: "u", sk: "a0" }],
    delete: [],
  }
  const out = applyBlockPatch(content, meta, patch, 1000)
...
test("合并 结合律: merge(merge(A,B),C) ≡ merge(A,merge(B,C)) (meta)", () => {
  ...
  const B: [Block[], BlockMetaMap] = [
    [blk("B1", "B1"), blk("B2", "B2")],
    { B1: m(2, "b", "a0"), B2: m(1, "b", "a1") },
  ]
