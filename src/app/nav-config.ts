import { Bookmark, Bot, FolderOpen, LayoutDashboard, Map, Megaphone, Newspaper, Rss, Wrench } from "lucide-react"
import type { ComponentType } from "react"

/** 导航单一真相源 —— 同时驱动桌面头部、移动 Sheet、⌘K 命令台, 杜绝手抄漂移。 */
export type NavLink = {
  href: string
  label: string
  icon: ComponentType<{ className?: string }>
  /** spoke 分类色 (Tailwind bg-* 类), 仅用于小圆点 */
  dot?: string
}

export const HUB_HREF = "/home"
export const HUB_LABEL = "我的空间"

/** 三条 spoke (发现): 视觉从属于中枢, 各带一个分类色点。 */
export const SPOKES: NavLink[] = [
  { href: "/info", label: "资讯", icon: Newspaper, dot: "bg-spoke-info" },
  { href: "/community", label: "社区", icon: Map, dot: "bg-spoke-community" },
  { href: "/tool", label: "工具", icon: Wrench, dot: "bg-spoke-tool" },
]

/** 我的空间子区 (中枢内部分区), 供命令台 / 移动菜单跳转。 */
export const HOME_SUBPAGES: NavLink[] = [
  { href: "/home", label: "概览", icon: LayoutDashboard },
  { href: "/home/subscriptions", label: "订阅", icon: Rss },
  { href: "/home/agent", label: "AI 助手", icon: Bot },
  { href: "/home/publications", label: "我的发布", icon: Megaphone },
  { href: "/home/resources", label: "资源管理", icon: FolderOpen },
  { href: "/home/bookmarks", label: "书签管理", icon: Bookmark },
]
