...
function childrenSorted(puts: Note[], parentId: string | null): Note[] {
  const kids = puts.filter((n) => n.parentId === parentId).sort((a, b) => (a.sortKey < b.sortKey ? -1 : 1))
  for (let i = 1; i < kids.length; i++) {
    assert.ok(kids[i - 1].sortKey < kids[i].sortKey, `同级 sortKey 应严格递增: ${kids[i - 1].sortKey} < ${kids[i].sortKey}`)
  }
...
test("已迁移 (笔记有 sortKey、无 notebookId, 笔记本已空) → null (幂等)", () => {
  const migrated = [
    { id: "n1", title: "a", content: [], parentId: null, sortKey: "a0", tags: [], createdAt: 1, updatedAt: 1 },
  ]
...
test("笔记本 + 子笔记: 目录页复用 notebook.id, 子笔记 parentId 零重指, 数据保留", () => {
  const notes = [
    { id: "n1", title: "子1", content: [{ type: "p", children: [{ text: "正文1" }] }], notebookId: "nb1", tags: ["x"], createdAt: 10, updatedAt: 20 },
    { id: "n2", title: "子2", content: [], notebookId: "nb1", tags: [], createdAt: 11, updatedAt: 30, deletedAt: 99 },
  ]
...
test("根级混排: 目录页与未分组笔记同组, sortKey 不碰撞且目录页在前", () => {
  const notes = [
    { id: "u1", title: "未分组A", content: [], notebookId: null, tags: [], createdAt: 1, updatedAt: 100 },
    { id: "u2", title: "未分组B", content: [], notebookId: null, tags: [], createdAt: 2, updatedAt: 50 },
  ]
  const notebooks = [
...
test("孤儿笔记 (notebookId 指向不存在笔记本) 归根", () => {
  const notes = [
    { id: "o1", title: "孤儿", content: [], notebookId: "ghost", tags: [], createdAt: 1, updatedAt: 1 },
  ]
...
test("崩溃重跑: 已迁移节点不重建, 但仍返回待清旧笔记本 (幂等收尾)", () => {
  ...
  const rawNotes = [
    { id: "nb1", title: "本子", content: [{ type: "p", children: [{ text: "" }] }], parentId: null, sortKey: "a0", tags: [], createdAt: 5, updatedAt: 5 },
    { id: "n1", title: "子", content: [], parentId: "nb1", sortKey: "a1", tags: [], createdAt: 10, updatedAt: 20 },
  ]
