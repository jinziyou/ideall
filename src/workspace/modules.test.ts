...
test("descriptorForPath: 精确匹配各模块面板路由", () => {
  ...
  assert.equal(descriptorForPath("/apps/local-apps")?.kind, "file-engine")
  ...
  assert.equal(descriptorForPath("/activity/deleted")?.kind, "file-engine")
})
...
test("descriptorForPath: 规范目录 URL 生成规范 file-engine descriptor", () => {
  ...
  assert.equal(descriptorForPath("/home/subscriptions")?.kind, "home-overview")
  assert.equal(descriptorForPath("/apps"), null)
  assert.equal(descriptorForPath("/trash"), null)
})

test("descriptorForPath: 前缀回退", () => {
  assert.equal(descriptorForPath("/home/subscriptions")?.kind, "home-overview")
  assert.equal(descriptorForPath("/home/settings/anything")?.kind, "home-overview")
...
test("descriptorForResource: 只接受当前 ?resource 深链", () => {
  const fileRef = {
    scheme: "node",
    kind: "file",
    id: "a:b/c?d&e=f",
  ...
  assert.equal(descriptorForResource("?node=note:abc123"), null)
  ...
  assert.equal(descriptorForResource("?other=1"), null, "无 resource 参数")
