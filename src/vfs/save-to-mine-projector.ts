...
function canWriteProjection(ctx: VfsAccessContext, projection: SaveToMineProjection): boolean {
...
export async function saveResourceToMine(
  ...
  if (!projection) {
    throw new VfsError("unsupported", `Resource cannot be saved to mine: ${ref.scheme}:${ref.kind}`)
  }
  if (!canWriteProjection(ctx, projection)) {
    throw new VfsError("permission-denied", "Missing permission to save resource to mine")
  }
