...
import {
  createConnectedResourceSources,
  infoResourceSource,
...
test("connected providers: route resources list/get/invoke without wire DTOs", async () => {
  const tools = await toolResourceSource.list({ scheme: "tool", text: "AI" }, ctx)
  assert.deepEqual(
  ...
  const entity = await infoResourceSource.get(entityRef, ctx)
  ...
  assert.deepEqual(await infoResourceSource.invoke(entityRef, "navigate", null, ctx), {
    ref: entityRef,
...
test("connected providers: reject invalid kinds and expose shared route mapping", async () => {
  await rejectCode(toolResourceSource.list({ scheme: "tool", kind: "entity" }, ctx), "unsupported")
  assert.equal(
...
test("connected providers: list followed entity and peer resources from subscriptions", async () => {
  ...
  const [info, community] = createConnectedResourceSources({
...
test("connected providers: list local bookmarks as browser resources", async () => {
  ...
  const providers = createConnectedResourceSources({
...
test("connected providers: invoke save-to-mine through injected projector", async () => {
  ...
  const [info] = createConnectedResourceSources({
    async listSubscriptionsByTypes() {
      return []
    },
...
test("connected providers: watch only forwards matching subscription updates", () => {
  withWindow((target) => {
    let count = 0
    const handle = infoResourceSource.watch!({ scheme: "info", kind: "entity" }, ctx, () => count++)
    assert.ok(handle)
...
test("connected providers: static resources do not subscribe to file updates", () => {
  assert.equal(
    toolResourceSource.watch?.({ scheme: "tool", kind: "search" }, ctx, () => {}),
    null,
  )
})
