...
export { routeForConnectedResource } from "@/lib/connected-resource"
...
export type ConnectedResourceSourceDeps = {
...
const defaultDeps: ConnectedResourceSourceDeps = {
...
function knownRecord(ref: ResourceRef, title?: string): ConnectedRecord {
  ...
  if (!record)
    throw new ResourceSourceError("unsupported", `Unsupported route resource: ${resourceKey(ref)}`)
  return record
}
...
function queryKinds(query: ResourceQuery, scheme: RouteScheme): string[] | null {
  ...
  for (const kind of rawKinds) {
    if (!isResourceKindForScheme(scheme, kind)) {
      throw new ResourceSourceError("unsupported", `Unsupported ${scheme} kind: ${kind}`)
    }
  }
...
function createRouteProvider(
  ...
  saveToMine?: ConnectedResourceSourceDeps["saveResourceToMine"],
): ResourceSourceProvider {
  ...
  const recordForRef = (ref: ResourceRef): ConnectedRecord | null => {
    if (ref.scheme !== scheme || !isResourceKindForScheme(scheme, ref.kind)) {
      throw new ResourceSourceError(
        "unsupported",
        `Unsupported ${scheme} resource: ${resourceKey(ref)}`,
      )
    }
  ...
  return {
    ...
    async invoke(ref, action, input, ctx) {
      const record = recordForRef(ref)
      if (!record)
        throw new ResourceSourceError("not-found", `Resource not found: ${resourceKey(ref)}`)
...
export function createConnectedResourceSources(
  deps: Partial<ConnectedResourceSourceDeps> = {},
): ResourceSourceProvider[] {
  const resolvedDeps: ConnectedResourceSourceDeps = { ...defaultDeps, ...deps }
  const infoResourceSource = createRouteProvider(
    "info",
    INFO_RECORDS,
    async (query) => {
  ...
  const communityResourceSource = createRouteProvider(
    "community",
    COMMUNITY_RECORDS,
    async (query) => {
      const kinds = query.kinds ?? (query.kind != null ? [query.kind] : ["peer"])
...
const defaultConnectedProviders = createConnectedResourceSources()

export const [
  infoResourceSource,
  communityResourceSource,
  toolResourceSource,
  browserResourceSource,
  appResourceSource,
] = defaultConnectedProviders

export const connectedResourceSources: ResourceSourceProvider[] = defaultConnectedProviders
