...
async function startEchoHttp() {
  const server = new McpServer({ name: "echo-http-test", version: "1.0.0" })
  let calls = 0
  server.tool("echo", { text: z.string() }, async ({ text }) => {
    calls += 1
    return { content: [{ type: "text", text: JSON.stringify({ echoed: text }) }] }
...
function externalEcho(url: string) {
  return {
    ...
    externalServers: [
      { id: "x", name: "Echo", transport: "sse" as const, url, configRevision: 11 },
    ],
  }
}
...
test("外部 MCP (SSE) 真连接: 列出工具(m0_ 前缀) 且 callTool 真执行", async () => {
  ...
  try {
    ...
    const diagnostic = diagnosticForMcpServer("x", 11)
    assert.equal(diagnostic?.status, "connected")
    assert.equal(diagnostic?.activeSessions, 1)
    assert.equal(diagnostic?.lastCall?.toolName, "echo")
    assert.equal(diagnostic?.lastCall?.status, "success")
  } finally {
    await mcp.close()
    assert.equal(diagnosticForMcpServer("x", 11)?.status, "healthy")
    await echo.close()
  }
})
...
test("外部 MCP (Streamable HTTP) 使用同一健康与最近调用诊断", async () => {
  clearMcpDiagnostics()
  const echo = await startEchoHttp()
  ...
  try {
    assert.equal(mcp.diagnostics.length, 0, JSON.stringify(mcp.diagnostics))
    const result = await mcp.callTool("m0_echo", { text: "hi" })
    assert.deepEqual(result, { ok: true, data: { echoed: "hi" } })
    assert.equal(echo.getCalls(), 1)
    const diagnostic = diagnosticForMcpServer("http-echo", 21)
...
test("外部 MCP 连接失败: 返回诊断而非静默吞掉", async () => {
    ...
    externalServers: [
      {
        ...
        name: "Bad MCP",
        transport: "sse",
        url: "not-a-url?token=private-secret",
        configRevision: 12,
      },
    ],
  })
  try {
    ...
    assert.equal(mcp.diagnostics[0].failureCode, "invalid-configuration")
    assert.equal(diagnosticForMcpServer("bad", 12)?.status, "error")
    assert.doesNotMatch(JSON.stringify(getMcpDiagnostics()), /private-secret|not-a-url/)
  } finally {
    await mcp.close()
  }
})

test("stdio MCP 在非桌面宿主返回统一的平台不支持诊断", async () => {
  clearMcpDiagnostics()
  ...
  assert.equal(result.errorCode, "transport-unsupported")
  assert.doesNotMatch(JSON.stringify(result), /private-command|private-secret|--token/)
  assert.equal(diagnosticForMcpServer("stdio-web", 31)?.status, "error")
})

test("Agent 持久工具事件只接收脱敏 MCP 连接诊断", async () => {
  const llm = await startMockLlm([finalResp])
  try {
        ...
        externalServers: [
          {
            ...
            name: "Unsafe\nMCP",
            transport: "http",
            url: "not-a-url?token=private-diagnostic-secret",
            configRevision: 41,
          },
        ],
      },
    })
    const serialized = JSON.stringify(result.toolEvents)
    assert.match(serialized, /MCP 连接配置无效/)
    assert.doesNotMatch(serialized, /private-diagnostic-secret|not-a-url|\n/)
  } finally {
    await llm.close()
  }
})
...
test("runAgent 端到端: 模型调用外部工具 → 真执行 → 最终答复", async () => {
  ...
  try {
    const res = await runAgent({
      ...
      onToolIntent: async () => {
        assert.equal(echo.getCalls(), 0, "pending outbox 必须先于副作用落盘")
        auditOrder.push("pending")
        return "audit-1"
      },
      onToolAudit: ({ auditId, status }) => {
        assert.equal(echo.getCalls(), 1)
        assert.equal(auditId, "audit-1")
        auditOrder.push(status)
      },
    })
...
test("耐久审计意图写入失败: 阻止外部工具且继续完成模型回合", async () => {
  const echo = await startEchoSse()
  const llm = await startMockLlm([toolCallResp, finalResp])
  try {
    const res = await runAgent({
    ...
    assert.equal(echo.getCalls(), 0)
    assert.ok(
      res.toolEvents.some((event) => event.summary.includes("耐久审计") && !event.ok),
      "应向用户和模型明确说明工具未执行",
    )
    assert.equal(res.content, "完成")
  } finally {
...
test("工具审批 confirm: onApprove 返回 false → 工具被跳过, 不产生副作用", async () => {
  ...
  try {
    ...
    assert.equal((approvalPreview as { risk?: unknown }).risk, "high")
    assert.doesNotMatch(JSON.stringify(approvalPreview), /"hi"/)
    assert.equal((audits[0] as { status?: unknown }).status, "rejected")
    assert.doesNotMatch(JSON.stringify(res.toolEvents), /"hi"/)
    assert.equal(res.content, "完成")
  } finally {
    await llm.close()
    await echo.close()
