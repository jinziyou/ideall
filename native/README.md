...
- `ideall-mobile`：gpui-mobile iOS/Android 静态/动态库入口，并包含 Android Gradle 与 iOS XcodeGen 工程壳。编辑器变换收口在 `src/editor.rs`，系统文本输入桥收口在 `src/native_text.rs`。Android 需要 SDK、NDK、`cargo-ndk` 和 Gradle 8.9+；iOS 需要 Xcode 16 与 XcodeGen。真机签名通过 `IDEALL_DEVELOPMENT_TEAM` 传入。
