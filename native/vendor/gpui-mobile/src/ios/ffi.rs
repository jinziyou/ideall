...
pub extern "C" fn gpui_ios_request_frame(window_ptr: *mut c_void) {
    ...
    let window = unsafe { &*(window_ptr as *const super::window::IosWindow) };
    let Some(_frame_guard) = window.begin_frame() else {
        log::warn!("GPUI iOS: dropping a concurrent or nested frame tick");
        return;
    };
    ...
    let callback = match window.request_frame_callback.try_borrow_mut() {
        ...
        Err(_) => {
            log::warn!("GPUI iOS: dropping a frame while its callback is already borrowed");
            return;
        }
    };
    if let Some(mut cb) = callback {
        ...
        match window.request_frame_callback.try_borrow_mut() {
            Ok(mut callback) => {
                if callback.is_none() {
                    *callback = Some(cb);
                }
            ...
            Err(_) => {
                log::error!("GPUI iOS: frame callback could not be restored");
            }
        }
    }
}
