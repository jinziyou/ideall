...
pub(crate) struct IosWindow {
    ...
    /// Prevents concurrent or nested FFI frame pumps from aliasing RefCell state.
...
impl IosWindow {
    ...
    pub(crate) fn pump_momentum(&self) {
        let (position, delta, fling_ended) = {
            let Ok(mut scroller) = self.momentum_scroller.try_borrow_mut() else {
                log::warn!("GPUI iOS: skipping a frame while momentum state is already borrowed");
                return;
            };
            ...
            if let Some(delta) = scroller.step() {
                (
                    ...
                    Some(gpui::point(gpui::px(delta.dx), gpui::px(delta.dy))),
                    !scroller.is_active(),
                )
            } else {
                (
        ...
        let Ok(mut callback) = self.input_callback.try_borrow_mut() else {
            log::warn!("GPUI iOS: skipping momentum input while its callback is already borrowed");
            return;
        };
    ...
    /// Claims the window's frame pump until the returned guard is dropped.
    pub(super) fn begin_frame(&self) -> Option<IosFrameGuard<'_>> {
        self.frame_in_progress
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .ok()
            .map(|_| IosFrameGuard {
                in_progress: &self.frame_in_progress,
            })
    }
...
pub(super) struct IosFrameGuard<'a> {
    in_progress: &'a AtomicBool,
}

impl Drop for IosFrameGuard<'_> {
    fn drop(&mut self) {
