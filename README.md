...
```bash
pnpm verify:checks        # 不含构建：format / lint（代码、workflow、依赖、死文件、文档）/ version / typecheck / test / API drift
