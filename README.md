...
终端的四个分区里，**「我的」(home) 是你本机数据的所在**；`info`（资讯）/ `community`（社区）/ `tool`（工具）是三个**发现**模块，它们发现的内容经**关注汇入「我的」**：

| 模块 | 路由 | 角色 | 是否依赖远程后端 |
| --- | --- | --- | --- |
| 我的 (home) | `/home` | 本机数据区：笔记 / 书签 / 资源 / 关注 / 对话，本地优先 | 否 |
| info | `/info` | 信息聚合展示 | 是（后端数据服务） |
| community | `/community` | 发布者地图、关注与发布 | 是（后端数据服务） |
...
| **与 wonita 本地联调** | 在 **jinziyou 工作区根** 用 `local-dev/bridge.sh`（见上级 `local-dev/README.md`），勿在 ideall 仓内写 localhost 默认值 |
